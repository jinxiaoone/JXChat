...
#import "JXDBGroupSQL.h"

@implementation JXDBGroupStore
...
- (BOOL)addGroup:(JXGroup *)group forUid:(NSString *)uid
{
    ...
    NSArray *arrPara = [NSArray arrayWithObjects:
                        JXNoNilString(uid),
                        JXNoNilString(group.groupID),
                        JXNoNilString(group.groupName),
                        @"", @"", @"", @"", @"", nil];
...
- (BOOL)updateGroupsData:(NSArray *)groupData forUid:(NSString *)uid
{
    ...
    if (oldData.count > 0) {
        ...
        for (JXGroup *group in groupData) {
            [newDataHash setValue:@"YES" forKey:group.groupID];
        }
        for (JXGroup *group in oldData) {
...
- (NSMutableArray *)groupsDataByUid:(NSString *)uid
{
    ...
    [self excuteQuerySQL:sqlString resultBlock:^(FMResultSet *retSet) {
        while ([retSet next]) {
            JXGroup *group = [[JXGroup alloc] init];
...
- (BOOL)addGroupMember:(JXUser *)user forUid:(NSString *)uid andGid:(NSString *)gid
{
